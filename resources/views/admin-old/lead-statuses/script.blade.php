<script>
    $(document).ready(function () {
        ...
        function setDataAndShowEditModel(data) {
            ...
            $('#update-form').attr('action', `{{route('admin.lead-status.update')}}/` + data.id);
        ...
        $('.change-status').on('change', function () {
            AjaxRequestPromise(`{{ route('admin.lead-status.change.status') }}/${$(this).data('id')}?status=${+$(this).is(':checked')}`, null, 'GET', {useToastr: true})
                .then(response => {
                })
