<script>
    $(document).ready(function () {
        ...
        $('.change-status').on('change', function () {
            AjaxRequestPromise(`{{ route('developer.employee.change.status') }}/${$(this).data('id')}?status=${+$(this).is(':checked')}`, null, 'GET', {useToastr: true})
                .then(response => {
                })
        ...
        $(document).on('click', '.deleteBtn', function () {
            ...
            AjaxDeleteRequestPromise(`{{ route("developer.employee.delete", "") }}/${id}`, null, 'DELETE', {
                useDeleteSwal: true,
                useToastr: true,
            })
                .then(response => {
