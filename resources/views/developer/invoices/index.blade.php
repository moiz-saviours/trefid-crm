...
@push('breadcrumb')
    <li class="breadcrumb-item text-sm text-white active" aria-current="page"><a
            href="{{route('developer.invoice.index')}}">Invoice</a>
    </li>
@endpush
@section('content')
    @push('style')
            ...
            <div class="col-12">
                <div class="card mb-4">
                    <div class="card-header pb-0">
                        ...
                        <a href="{{ route('developer.invoice.create') }}"
                                    ...
                                    <tr id="tr-{{$invoice->id}}">
                                        ...
                                        <td class="align-middle text-center text-nowrap">
                                            @if(isset($invoice->brand))
                                                <a href="{{route('developer.brand.edit',[$invoice->brand->id])}}">{{ $invoice->brand->name }}</a>
                                        ...
                                        <td class="align-middle text-center text-nowrap">
                                            @if(isset($invoice->team))
                                                <a href="{{route('developer.team.edit',[$invoice->team->id])}}">{{ $invoice->team->name }}</a>
                                        ...
                                        <td class="align-middle text-center text-nowrap">
                                            @if(isset($invoice->client))
                                                <a href="{{route('developer.client.edit',[$invoice->client->id])}}">{{ $invoice->client->name }}</a>
                                        ...
                                        <td class="align-middle text-center text-nowrap">
                                            @if(isset($invoice->agent))
                                                <a href="{{route('developer.employee.edit',[$invoice->agent->id])}}">{{ $invoice->agent->name }}</a>
                                            @else
                                                ---
                                            @endif
                                        </td>
                                        ...
                                        <td class="align-middle text-center table-actions">
                                            <a href="{{ route('developer.invoice.edit', [$invoice->id]) }}"
