<script>
    $(document).ready(function () {
        ...
        {{--    AjaxRequestPromise(`{{ route('developer.lead.change.status') }}/${$(this).data('id')}?status=${+$(this).is(':checked')}`, null, 'GET', {useToastr: true})--}}
        {{--        .then(response => {--}}
        {{--        })--}}
        ...
        $(document).on('click', '.deleteBtn', function () {
            ...
            AjaxDeleteRequestPromise(`{{ route("developer.lead.delete", "") }}/${id}`, null, 'DELETE', {
                useDeleteSwal: true,
                useToastr: true,
            })
                .then(response => {
                ...
                .catch(error => {
                    if (error.isConfirmed === false) {
                        Swal.fire('Action Canceled', 'Phew! The delete action was saved.', 'info');
                    } else {
