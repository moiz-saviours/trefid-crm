    ...
    $(document).ready(function () {
        ...
        function initializeDatatable(table_div, index) {
            ...
            $(document).on('click', '.editBtn', function () {
                ...
                $('#manage-form')[0].reset();
                $.ajax({
                    url: `{{route('user.client.contact.edit')}}/` + id,
                    type: 'GET',
            ...
            function setDataAndShowEdit(data) {
                ...
                $('#manage-form').attr('action', `{{route('user.client.contact.update')}}/` + client_contact.id);
            ...
            $('#manage-form').on('submit', function (e) {
                ...
                if (!dataId) {
                    AjaxRequestPromise(`{{ route("user.client.contact.store") }}`, formData, 'POST', {useToastr: true})
                        .then(response => {
                            if (response?.data) {
        ...
        $('tbody').on('change', '.change-status', function () {
            ...
            let table = dataTables[0];
            AjaxRequestPromise(`{{ route('user.client.contact.change.status') }}/${rowId}?status=${status}`, null, 'GET', {useToastr: true})
                .then(response => {
